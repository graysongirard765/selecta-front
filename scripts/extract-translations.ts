...
const outputFile = path.resolve(messagesDir, `${sourceLocale}.json`);
...
const sourceLocaleMessages = mergeTranslations(
  extractedTranslations,
  readJsonFile(outputFile)
);
fs.writeFileSync(outputFile, JSON.stringify(sourceLocaleMessages, null, 2), 'utf8');
...
console.log(
  `✅ Translations written to ${outputFile} and synced to ${localeFiles.length} locale files`
);
